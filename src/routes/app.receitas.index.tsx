...
function RecipesPage() {
  ...
  const load = async () => {
    if (!user) return;
    const { data } = await supabase.from("recipes").select("id,name,servings").eq("user_id", user.id).order("created_at", { ascending: false });
    setList(data ?? []);
  };
  ...
  const create = async () => {
    ...
    const { error } = await supabase.from("recipes").insert({ user_id: user.id, name: name.trim(), servings });
    if (error) return toast.error(error.message);
    setName(""); setServings(1); setOpen(false); load();
  };
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button size="sm" className="rounded-full"><Plus className="h-4 w-4 mr-1" />Nova</Button></DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle>Nova receita</DialogTitle></DialogHeader>
            <div className="space-y-3">
              <div><Label>Nome</Label><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Frango com batata" autoFocus /></div>
              <div><Label>Porções</Label><Input type="number" value={servings} onChange={(e) => setServings(Number(e.target.value))} /></div>
            </div>
      ...
      {list.length === 0 ? (
        <Card className="p-10 text-center">
          ...
          <p className="text-muted-foreground">Salve receitas e adicione com 1 toque às refeições.</p>
        </Card>
      ) : (
