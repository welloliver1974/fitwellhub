...
import { coachAdvice } from "@/server/nutrition.functions";
...
function CoachPage() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const [{ data: goals }, { data: meals }, { data: workouts }, { data: weights }, { data: water }] = await Promise.all([
        supabase.from("goals").select("calories,protein_g,carbs_g,fat_g").eq("user_id", user.id).maybeSingle(),
        supabase.from("meals").select("id,meal_date,meal_type").eq("user_id", user.id).gte("meal_date", start).lte("meal_date", today),
        supabase.from("workouts").select("id,name,workout_date").eq("user_id", user.id).gte("workout_date", start).lte("workout_date", today),
      ...
      for (const m of meals ?? []) {
        ...
        const tot = items.filter((i) => i.meal_id === m.id).reduce((a, i) => ({
          kcal: a.kcal + i.calories, p: a.p + i.protein_g, c: a.c + i.carbs_g, f: a.f + i.fat_g, water: 0,
      ...
      const lines = days.map(([d, t]) => `- ${d}: ${Math.round(t.kcal)} kcal · P${Math.round(t.p)} C${Math.round(t.c)} G${Math.round(t.f)} · água ${Math.round(t.water)}ml`);
  ...
  return (
    <div className="space-y-5">
      ...
      <Button onClick={generate} disabled={loading} className="w-full">
        {loading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Analisando…</> : <><Sparkles className="h-4 w-4 mr-2" /> Gerar análise da semana</>}
      </Button>
      {text && (
        <Card className="p-5 whitespace-pre-wrap text-sm leading-relaxed">{text}</Card>
      )}
      {!text && !loading && (
