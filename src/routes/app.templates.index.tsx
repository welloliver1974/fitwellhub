...
function TemplatesPage() {
  ...
  const load = async () => {
    if (!user) return;
    const { data: t } = await supabase.from("workout_templates").select("id,name").eq("user_id", user.id).order("created_at");
    setTemplates(t ?? []);
    const ids = (t ?? []).map((x) => x.id);
    if (ids.length) {
      const { data: e } = await supabase.from("workout_template_exercises").select("id,template_id,name,position").in("template_id", ids).order("position");
      setExs((e ?? []) as TplEx[]);
    } else setExs([]);
  };
  ...
  const create = async () => {
    ...
    const { error } = await supabase.from("workout_templates").insert({ user_id: user.id, name: name.trim() });
  ...
  const useTemplate = async (tpl: Template, e: React.MouseEvent) => {
    e.preventDefault(); e.stopPropagation();
    if (!user) return;
    const today = new Date().toISOString().slice(0, 10);
    const { data: w, error } = await supabase.from("workouts")
    ...
    if (tplExs.length) {
      await supabase.from("exercises").insert(
        tplExs.map((x) => ({ user_id: user.id, workout_id: w.id, name: x.name, position: x.position }))
      );
    }
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button size="sm" className="rounded-full"><Plus className="h-4 w-4 mr-1" />Novo</Button></DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle>Novo template</DialogTitle></DialogHeader>
            <div className="space-y-3">
              <Label>Nome</Label>
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Treino A — Peito/Tríceps" autoFocus />
            </div>
              ...
              <Link key={t.id} to="/app/templates/$id" params={{ id: t.id }} className="block">
                <Card className="p-4 flex items-center justify-between hover:bg-secondary/50 transition-colors">
                  ...
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon" onClick={(e) => useTemplate(t, e)} title="Usar hoje">
