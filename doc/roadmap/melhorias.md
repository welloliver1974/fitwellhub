...
### 3. ✅ Cinco baterias entregues (02/08/2026) - Criar testes de avaliação da IA
Antes nada era testado. **Vitest** configurado e a lógica pura foi extraída para módulos testáveis (`src/lib/coach-plan.ts`, `src/lib/food-utils.ts`, `src/lib/ai-settings.ts`) + componentes extraídos (`src/components/plan-card.tsx`, `reminders-page.tsx`, `goals-page.tsx`). **69 testes verdes** em cinco baterias:
...
- **5ª bateria (jsdom, integração de páginas de CRUD):** 9 testes — `RemindersPage` (5: lista, add, sem-dias, toggle, delete) e `GoalsPage` (4: pré-carrega meta, macroKcal, salvar→upsert+navigate, aviso>50). **Refactor associado:** `RemindersPage`/`GoalsPage` movidas das rotas para `src/components/` — **exportar do arquivo de rota dispara warning do TanStack Router sobre perder code-split**.
